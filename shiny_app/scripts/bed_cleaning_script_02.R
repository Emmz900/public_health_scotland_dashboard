...
beds <- janitor::clean_names(read_csv("shiny_app/raw_data/beds_by_nhs_board_of_treatment_and_specialty.csv"))

# added to replace health board codes with location name
health_board <- janitor::clean_names(read_csv("shiny_app/raw_data/health_board.csv")) %>% 
  select(hb, hb_name)

# added to replace specialist board codes with location name
specialist_board <- janitor::clean_names(read_csv("shiny_app/raw_data/special_board.csv")) %>% 
  select(shb, shb_name)

# added to replace the hospital codes with location name
hospital_data <- janitor::clean_names(read_csv("shiny_app/raw_data/hospital.csv")) %>% 
...
beds_clean <- select(beds_clean, -hb, -specialty, -location, -quarter_qf, -hbqf,
                     -location_qf, -specialty_qf, -specialty_name_qf,
                     -all_staffed_beddays_qf, -average_available_staffed_beds_qf,
                     -average_occupied_beds_qf, -percentage_occupancy_qf,
                     -total_occupied_beddays_qf)
# reordering cols
beds_clean <- select(beds_clean, year, quarter, location_name, hb_name, everything())

# removing unneeded objects from environment 
rm(beds,health_board,hospital_data,specialist_board)

write_csv(beds_clean, "shiny_app/clean_data/clean_bed_data.csv")